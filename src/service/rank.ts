...
${articleList}
`
    return llmRankArticles(prompt)
}
async function classifyScoresRank(articleList: any[]): Promise<any[]> {
    const orderArticle: any[] = [];
    const scoreGeq9: any[] = [];
    const scoreEq8: any[] = [];
    const scoreLeq7: any[] = [];
    ...
    // 处理分数为 8 的文章
    if (scoreEq8.length >= LIMIT_SEND_COUNT - scoreGeq9.length) {
        const scoreEq8Rank = scoreEq8.map((item, idx) => ({ index: idx, title: item.title, summary: item.article_abstract }));
        const llmRankList = await rankArticles(JSON.stringify(scoreEq8Rank, null, 2));
